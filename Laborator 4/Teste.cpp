...
void testDomain() {
	...
	Tranzactie tr2(2, 2, 10, "da", "amenda");
	...
	Tranzactie tr3(3, 3, 10, "da", "amenda");
	t = tr3;
	assert(t == tr3);
	t = t;
	assert(t == t);
...
void testRepository() {
	Tranzactie t1(1, 1, 1, "out", "amenda");
	Tranzactie t2(2, 2, 2, "in", "salar");
	Tranzactie t3(3, 3, 3, "out", "cumparaturi");
	Tranzactie t4(4, 4, 4, "in", "imprumut");
	Tranzactie t_neexistent(100, 1, 1, "out", "amenda");
	RepositorySTL rep;
	...
	Tranzactie t5(5, 5, 5, "out", "datorie");
	...
	char* q = new char[6];
	strcpy_s(q, 6, "salar");
	Tranzactie _t_n(1, 10, 10, p, q);
	Tranzactie t1_nou = rep.updateElem(_t_n);

	assert(strcmp(t1_nou.getTip(), "in") == 0);
	assert(strcmp(t1_nou.getDescriere(), "salar") == 0);

	t1_nou = rep.updateElem(t_neexistent);
...
void testService() {
	...
	service.addTranzactie(t1.getId(), t1.getZiua(), t1.getSuma(), t1.getTip(), t1.getDescriere());
	service.addTranzactie(t2.getId(), t2.getZiua(), t2.getSuma(), t2.getTip(), t2.getDescriere());
